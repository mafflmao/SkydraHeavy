...
namespace LibSkydra.Archive
{
	public class igArchive
	{
		...
		public enum IG_CORE_ARCHIVE_VERSION : uint
		{
			...
			si_nx = 0x0C,										// Skylanders Imaginators on Nintendo Switch*, version number is 0xDEAD cos idk what it is
																// It's 0x0C :)
		}
		...
		{
			sh = new StreamHelper(File.Open(path, FileMode.Open, FileAccess.ReadWrite));
		...
		public void ReadHeader()
		{
			...
			for(uint i = 0; i < numFiles; i++)
			{
				...
				if(version == IG_CORE_ARCHIVE_VERSION.ssf_not_citrus)
				{
		...
		public void ExtractFile(int i, Stream dst)
		{
			...
			for(uint currentBlockIndex = 0; currentBlockIndex < numBlocks; currentBlockIndex++)
			{
				...
				Console.WriteLine($"Decompressing {compressedSize.ToString("X08")} bytes as zlib from {names[i]} @ {sh.BaseStream.Position.ToString("X08")}");
